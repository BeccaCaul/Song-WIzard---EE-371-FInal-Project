/* video driver outputs an x and y cordinate and takes in a rgb value and send data to the VGA to control the red_to_vga_display
* this cycles through the cordinates, which the system can then send rgb values to the video driver for each cordinate as thet show up
* This works on a 25 MHz clock unlike the majority of the rest of the system
 */
module get_image
    #(parameter WIDTH  = 640,
    ...
    //next state to avoid timing issues with RAM
    logic [9:0] x_next;
    logic [8:0] y_next;
    ...
    // mif files loaded that we switch between ----------------------------------------
    ...
    //key fill when note is pressed 
    logic in_rect;
    
    //progress bar ------------------------------------------------------------------
    localparam [8:0] prog_Y0    = 9'd332;  
    localparam [8:0] prog_Y1    = 9'd339;   
    localparam [9:0] prog_X0 = 10'd205;
    localparam [9:0] prog_X1 = 10'd431;
    localparam RAM_DEPTH = 120;

    logic [9:0] bar_fill_x;
    always_comb begin
        bar_fill_x = prog_X0 + (RAM_addr * (prog_X1 - prog_X0)) / 120;  //scale ram addr
    end

    logic in_progress_bar;
    assign in_progress_bar = (y_next >= prog_Y0) && (y_next <= prog_Y1)
                          && (x_next >= prog_X0) && (x_next < bar_fill_x);
    ...
    always_comb begin
        ...
        green = 8'h00;
        blue  = 8'h00;
        if (in_progress_bar && !all_done) begin  //progress bar
            red   = 8'hFF;
            green = 8'hFF;
            blue  = 8'hFF;
        end
        else if(in_rect) begin
            if(A) begin
                red = 8'd195; 
                blue = 8'd216;
                green = 8'd125; 
